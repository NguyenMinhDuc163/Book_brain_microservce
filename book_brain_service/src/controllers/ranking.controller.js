...
exports.getBookRankings = async (req, res) => {
    try {
        ...
        res.status(500).json(createResponse('fail', 'Lỗi khi lấy bảng xếp hạng sách.', 500, []));
    }
};
...
exports.getAuthorRankings = async (req, res) => {
    try {
        ...
        res.status(500).json(createResponse('fail', 'Lỗi khi lấy danh sách tác giả xếp hạng cao.', 500, []));
    }
};
